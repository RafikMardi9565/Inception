...
2. For our named volumes (local driver with `device:`), Compose ensures the host path exists.
...
│      ↕                                               │
│  /home/rmardi/data/mariadb   ← host directory. persists │
│                                forever (until rm -rf) │
└──────────────────────────────────────────────────────┘
```
...
|------|--------|---------|---------------|
| **Bind mount** | Absolute host path → container path | `/home/rmardi/data/mariadb:/var/lib/mysql` | No — **forbidden by the subject** for these two volumes |
| **Named volume** | Volume name → container path | `mariadb_data:/var/lib/mysql` | Yes — **required**, with `driver_opts` pointing at `/home/rmardi/data/` |
| **Anonymous volume** | Container path only | `/var/lib/mysql` (no host side) | No — uncontrolled. |

### 5.3 Named Volume Stored in /home/<login>/data

The subject forbids bind mounts for the two persistent volumes and demands **named
volumes** whose data lives in `/home/<login>/data` on the host. The canonical pattern:
...
It is declared by name in the top-level `volumes:` section — a real named volume
(`docker volume ls` shows it), not a service-level bind mount (no `- /host:/path` under
any service). Each field:
...
| `o: bind` | Mount option telling the kernel to map the `device` directory |
| `device: /home/.../mariadb` | The absolute host directory the volume stores its data in |

On disk it behaves like a bind of `/home/rmardi/data/mariadb` (data lands directly in
that directory, which is what the subject's `/home/<login>/data` check looks at), but it
is declared and referenced as a named volume.

### 5.4 Short Syntax vs Long Syntax

```yaml
# Short syntax — reference the named volume:
volumes:
  - mariadb_data:/var/lib/mysql

# Long syntax — same thing, plus read-only for nginx:
volumes:
  - type: volume
...
Both reference the named volumes declared in the top-level `volumes:` section. The long
syntax is what lets you mark a mount read-only — used for NGINX's mount of the WordPress
volume (the subject's diagram shows NGINX mounting it `ro`; only WordPress needs write
access).
...
services:
  ...
  nginx:
    volumes:
      - wordpress_data:/var/www/html:ro  # SAME volume, read-only — nginx only serves
...
volumes:
  ...
  wordpress_data:
    ...
    driver_opts:
      type: none
      o: bind
      device: /home/rmardi/data/wordpress
```
...
- **Named volume, empty:** Docker **copies the image's content at that path into the
  volume** (copy-up / volume initialization). This applies to *any* real named volume —
  including our `o: bind`-style one, which is still a `Type: volume` mount, just with
  the data landing in `/home/rmardi/data/...`. Only plain bind mounts (`Type: bind`,
  i.e. `-v /host:/path`) are exempt — nothing is copied there.
...
**The Inception trap this creates (MariaDB):** `apt-get install mariadb-server` bakes a
*virgin datadir* into the image (`/var/lib/mysql` with system tables, `mysql/`, ibdata…).
First boot with an empty volume: Docker copy-up seeds that virgin datadir into the volume
→ the entrypoint's `if [ ! -d /var/lib/mysql/mysql ]` sees `mysql/` and thinks
"already initialized" → skips user/DB creation → WordPress gets error 1130 (no `wpuser`).
It only worked with manual `docker run -v` because those were *bind* mounts (no copy-up).
**Fix:** `RUN apt-get update && apt-get install -y mariadb-server && rm -rf /var/lib/mysql/*`
in the Dockerfile — the image ships an empty datadir, copy-up has nothing to copy, and the
entrypoint correctly detects "fresh" and initializes. (The same reason the official
mariadb image ships an empty datadir.)
...
WP_ADMIN_EMAIL=admin@rmardi.42.fr
...
- External traffic path: Browser → `https://rmardi.42.fr:443` → Host (VM) port 443 → NGINX container → `fastcgi_pass wordpress:9000` → `wp-config.php` → `DB_HOST=mariadb:3306`
...
2. How does `nginx:443` inside the NGINX container become reachable at `https://rmardi.42.fr` from the browser?
...
9. If the MariaDB data directory at `/home/rmardi/data/mariadb` is empty on first boot, what happens? Who populates it?
