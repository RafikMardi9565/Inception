...
│  - Data persisted to /home/rmardi/data/mariadb               │
│                                                                 │
...
http {
    ...
    server {
        ...
        server_name rmardi.42.fr;            # Domain this server block handles
...
Browser ── [encrypted blob] ────→ Server
                   ...
                   └── ISP: sees only that you visited rmardi.42.fr, nothing more
```
...
X.509 Certificate
─────────────────
Subject: CN=rmardi.42.fr, O=42 School, C=FR
Issuer:  CN=rmardi.42.fr, O=42 School, C=FR   ← Same as Subject = self-signed
...
Extensions:
  - Subject Alternative Name: DNS:rmardi.42.fr
...
| `/CN=rmardi.42.fr` | Common Name — the domain (legacy field; see SAN note below) |
...
- Browser: `https://rmardi.42.fr` → certificate warning → click Advanced → Proceed
- `curl -k -v https://rmardi.42.fr` shows:
  ```
  * SSL connection using TLSv1.3
  * Server certificate:
  *  subject: CN=rmardi.42.fr
...
support. Pin a specific wp-cli release — use the GitHub release URL above (note the `-L`
to follow its redirect; the old `gh-pages` versioned paths now return 404, and `curl`
would happily save that 404 page as a broken "binary" if you don't check).
...
- The evaluator visits `https://rmardi.42.fr` and sees a fully installed WordPress,
...
at `/var/www/html` because it's the same host directory (the named volume's `device:`) mounted into both containers.
...
```
  ...
  (Browser)         │                          │
                    ...
                    │                          │
                    │  /home/rmardi/data/         │
                    │  ├── wordpress/  ←─── Two named volumes, data in
                    │  └── mariadb/    ←─── /home/rmardi/data/
                    └─────────────────────────┘
```
...
7. Why is NGINX the only container with a published port (`443:443`)? What happens if you try to access `https://rmardi.42.fr:9000`?
