...
- [ ] **Configure the domain name**
  - [ ] Edit `/etc/hosts` so `rmardi.42.fr` points to `127.0.0.1` (or the VM's IP)
...
- [ ] Write an init script that on first run:
  ...
  - [ ] Installs WordPress (`wp core install`) with site URL `https://rmardi.42.fr`
...
- [ ] Write the Dockerfile
  ...
  - [ ] Generate (or copy a script that generates) the self-signed cert for `rmardi.42.fr`
...
- [ ] Test:
  - [ ] `https://rmardi.42.fr` loads WordPress (accept the self-signed cert warning)
  - [ ] `http://rmardi.42.fr` does NOT work
  - [ ] `curl -k -v https://rmardi.42.fr` shows TLSv1.2 or v1.3
...
- [ ] Define the 2 **named volumes** (bind mounts are FORBIDDEN by the subject!) with `driver_opts` pointing at `/home/rmardi/data/...`:
  - [ ] `wordpress` volume → `/home/rmardi/data/wordpress` (site files)
  - [ ] `mariadb` volume → `/home/rmardi/data/mariadb` (database)
  - [ ] Learn the `driver: local` + `driver_opts` (`type: none`, `o: bind`, `device:`) pattern — a named volume whose data lives in `/home/rmardi/data/`
- [ ] Write the `.env` file
...
- [ ] Write rules:
  - [ ] `all` / `up`: create `/home/rmardi/data/{wordpress,mariadb}` dirs, then `docker compose -f srcs/docker-compose.yml up -d --build`
  - [ ] `down`: `docker compose ... down`
  - [ ] `stop` / `start`
  - [ ] `clean`: down + remove images
  - [ ] `fclean`: clean + remove volumes + wipe `/home/rmardi/data` (careful with `sudo rm -rf`)
  - [ ] `re`: fclean + all
...
- [ ] Full validation pass:
  ...
  - [ ] Site loads at `https://rmardi.42.fr`, WordPress is installed (no install wizard shown)
