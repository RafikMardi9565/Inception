...
### 3.6 Why Daemonization Works on Real Systems — but Not in Containers

The daemonization ritual *depends* on an init system existing; in the container, that
ingredient is missing in the most fatal way possible.

**On a real machine, the dance works:**

```
nginx starts (child of your shell)
  → fork: nginx-child is born, nginx-parent EXITS
  → kernel ADOPTION rule: orphan → reparented to init (systemd)
  → nginx-child lives forever as systemd's child
```

The survival step is **adoption**: the exiting parent's child falls onto init, which
owns it from then on. That's why every traditional daemon ends up a direct child of
init — by design, it's the immortality trick.

**Inside the container, the same dance:**

```
nginx (PID 1 — the namespace's own "init")
  → fork: child born, PID 1 EXITS
  → kernel: "PID 1 of the namespace died" → TEARS DOWN THE WHOLE NAMESPACE
  → the forked child is killed in the teardown — before adoption could ever happen
```

A *different* kernel rule fires first: PID 1 death = namespace death. The survivor
never gets adopted because the entire world is deleted around it. The daemon didn't
need init to *function* — it needed init to *survive its own parent's exit*, and the
container's init WAS the parent that exited.

**The punchline:** the daemon doesn't need to daemonize in Docker at all, because the
platform already plays systemd's role:

```
Real system:   systemd        keeps the daemon alive, supervises, reaps, logs
Container:     containerd-shim keeps the container alive, supervises, reaps, logs
```

The shim is the container's init-analog. So we strip the ritual and run "naked" —
`daemon off;`, `-F`, plain `mysqld` — foreground, PID 1, and hand the keep-alive job
to the platform.

**The subtle case (the forbidden pattern in disguise):** if PID 1 is a *wrapper
script* and the daemon forks inside, the child IS adopted — by the wrapper. The
container lives, but now PID 1 is the wrapper (`sleep infinity`, `tail -f`): adoption
works, but onto the wrong parent — broken signals, no reaping, and the container
outlives its own service.

**The compact table — all three moves, seen by the shim:**

| | Who is PID 1? | If the service dies? | `docker stop`? | Logs? |
|---|---|---|---|---|
| Foreground (`exec`) | the service ✓ | container dies — honest, restart fires | SIGTERM → service, graceful | flow |
| Background (`&`) | the shell | container LIES (Up, site dead) | SIGTERM → shell, not forwarded, SIGKILL after 10s | flow (child inherits the mic) |
| Daemonize (fork+exit) | nobody — it exited | — | container died at birth (Exited 0) | silent, then dead |

One rule: **the shim watches PID 1 and only PID 1. Make PID 1 your service.**

And the `-d` clarification: `docker run -d` is NONE of these three — it's the `&`
move at the *terminal* level (start the container, don't block my shell, send output
to `docker logs`). Inside the container nothing changes: PID 1 is still whatever
`exec` made it. Your terminal is a messenger over the socket, never a parent — the
real family is systemd → containerd → shim → container PID 1.
